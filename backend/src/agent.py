...
class FoodAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'Robin', a helpful assistant for 'KSMart', an Indian grocery store.
            ...
            """,
            tools=[find_item, add_to_cart, remove_from_cart, update_cart_quantity, show_cart, add_recipe, place_order, cancel_order, get_order_status, order_history, ingredients_for],
        )
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🇮🇳" * 12)
    logger.info("🚀 STARTING KSMART (Indian Context + Auto-Tracking)")
