...
const MotionTitle = motion.create('h1'); // Component for the main title

const IN_DEVELOPMENT = process.env.NODE_ENV !== 'production';

// Motion props for the bottom controls area
...
// Motion props for the Game Title
const TITLE_MOTION_PROPS = {
    initial: { opacity: 0, scale: 0.8, y: -20 },
    animate: { opacity: 1, scale: 1, y: 0 },
    transition: {
        duration: 0.6,
        delay: 0.2, // Appears shortly after the Player Badge
        type: 'spring',
    },
};
...
// === BACKGROUND ICONS (Skipped for brevity, assumed included) ===
...
function PlayerBadge({ participant }: { participant?: LocalParticipant }) {
    ...
    useEffect(() => {
        ...
        const updateName = () => {
            ...
            // Fallback to 'Contestant' for the game show theme
...
export const SessionView = ({
    ...
    // Default chat to OPEN since this is a dialogue-focused application
    ...
    return (
        <section
            ...
            {...props}
        >
            {/* Background Gradient & Icons (Background logic assumed included) */}
            <div aria-hidden="true" className="absolute inset-0 -z-10 select-none overflow-hidden">
                ...
                {/* Floating Icons Background logic (assumed included) */}
            </div>

            {/* --- GAME TITLE ADDITION --- */}
            <MotionTitle
                {...TITLE_MOTION_PROPS}
                className="absolute top-10 left-1/2 -translate-x-1/2 z-40 text-4xl 
                           md:text-6xl font-black uppercase text-white tracking-widest 
                           drop-shadow-[0_0_15px_rgba(255,255,255,0.4)] whitespace-nowrap"
            >
                THE NARRATIVE NEXUS-Voice Improv Battle
            </MotionTitle>
